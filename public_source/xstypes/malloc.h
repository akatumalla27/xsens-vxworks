 /*
  ========================================================================
  This is a customized malloc.h header file for vxWorks.
  ========================================================================
*/
#ifndef MALLOC_272_H
#define MALLOC_272_H
...
/* vxWorks memory semaphore. */
static  SEMAPHORE        mem_semaphore;

#define MALLOC_PREACTION   semTake(&mem_semaphore, WAIT_FOREVER)
#define MALLOC_POSTACTION  semGive(&mem_semaphore)
...
/* #define MALLOC_ZERO(dest, nbytes)       bzero((char*)dest, nbytes) */
...
void *vxSbrk(int size)
{
    ...
    if(size)
    {
        ...
        if(!(tmp = memPartAlloc(memSysPartId, size+MALLOC_ALIGNMENT)))
            return (void*)-1;

        /* Must be aligned. */
        tmp = (void*)(((int)tmp + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK);
        previous_top = (void*)((int)tmp + size + MALLOC_ALIGNMENT);
        return tmp;
    }
